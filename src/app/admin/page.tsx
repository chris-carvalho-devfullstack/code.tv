...
export default async function AdminDashboard() {
  ...
  // Busca chaves vendidas OU reveladas usando o .in()
  const { count: chavesVendidas } = await supabase
    .from('chaves')
    .select('*', { count: 'exact', head: true })
    .in('status', ['VENDIDA', 'REVELADA']);

  // Busca o total absoluto de chaves no banco
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition">
          <h3 className="text-slate-500 font-medium mb-2 text-sm md:text-base">Total</h3>
