...
export default async function GerenciarChaves() {
  ...
  return (
    <div className="p-4 md:p-8">
      <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mb-6 md:mb-8">Estoque de Chaves</h1>

      {/* Formulário responsivo (colunas no mobile, linha no desktop) */}
      <div className="bg-white p-5 md:p-6 rounded-2xl border border-slate-200 shadow-sm mb-6 md:mb-8">
        <h2 className="text-lg font-bold text-slate-800 mb-4">Adicionar Nova Chave</h2>
        <form action={addChave} className="flex flex-col md:flex-row gap-4 md:items-end">
          ...
          <div className="flex-1">
            ...
            <select name="plano" required className="w-full px-4 py-2 border border-slate-300 rounded-lg outline-none bg-white focus:ring-2 focus:ring-blue-500 transition">
          ...
          <button type="submit" className="w-full md:w-auto bg-blue-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-blue-700 flex items-center justify-center gap-2 h-[42px] transition shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[600px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="p-4 font-semibold text-slate-600 text-sm">Código</th>
            ...
            <tbody>
              {chaves?.map((chave) => (
                <tr key={chave.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition">
                  <td className="p-4 font-mono text-sm text-slate-800">{chave.codigo}</td>
